...
export default function ProjectAtomCardComponent(props) {
    ...
    return(
        <div 
            className={`w-full md:w-28 lg:w-28 bg-${props.color?props.color:'blue'}-400  px-2 py-2 rounded-lg m-1 h-28`}
          >
            ...
            <div onMouseOver={()=>setDrop(false)} className="text-white mb-4 w-full text-center cursor-pointer">
              <div className="font-bold text-lg  md:text-medium">Dell UI</div>
              <div className="text-base md:text-xs  font-semibold">
                Feedbacks for our new interface
              </div>
            </div>
          </div>
    )
